...
final class VulkanAdvancedShaderBackend implements AdvancedShaderBackend {
    ...
    public void drawPatches(PatchDrawCommand command) {
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            rendering.renderArea().extent().set(color0.getWidth(0), color0.getHeight(0));
            ...
            try {
                ...
                viewport.get(0).x(0.0f).y(0.0f)
                        .width(color0.getWidth(0)).height(color0.getHeight(0))
                        .minDepth(0.0f).maxDepth(1.0f);
                vkCmdSetViewport(commandBuffer, 0, viewport);
                VkRect2D.Buffer scissor = VkRect2D.calloc(1, stack);
                scissor.get(0).offset().set(0, 0);
                scissor.get(0).extent().set(color0.getWidth(0), color0.getHeight(0));
    ...
    private void bindResourceDescriptors(VulkanCommandEncoder encoder,
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            for (StorageVolumeBinding binding : volumes) {
                if (!(binding.volume() instanceof VulkanStorageVolume volume)) {
                    throw new IllegalArgumentException("Storage volume does not belong to the active Vulkan backend");
                }
                volume.ensureGeneralLayout(commandBuffer);
                VkDescriptorImageInfo.Buffer info = VkDescriptorImageInfo.calloc(1, stack);
                info.get(0).sampler(0L)
