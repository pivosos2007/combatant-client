/*
 * This file is part of the Combatant Client distribution.
 * Copyright (c) 2026 pivosos2007.
 *
 * Licensed under the GNU General Public License v3.0.
 */

const n = ui.num;
const c = ui.str;
const s = ui.fmt;
const cls = ui.cls;
const color = ui.color.get;
const alpha = ui.color.alpha;

function font(family, size, type) {
  const suffix = type ? `-${type}` : "";
  return cls(`font-${family}${suffix}`, `font-size-${s(size)}`);
}

function entryPos(p, index) {
  const entryW = n(p.entryWidth, 24);
  const entryH = n(p.entryHeight, 24);
  const gap = n(p.gap, 4);
  if (c(p.direction, "horizontal") === "vertical") {
    return { x: 0, y: index * (entryH + gap), w: entryW, h: entryH };
  }
  return { x: index * (entryW + gap), y: 0, w: entryW, h: entryH };
}

function compactText(p, pal, pos, entry, index) {
  if (c(p.mode, "icons") !== "compact") return null;
  const fontSize = n(p.fontSize, 18 * 0.78);
  const widthScale = fontSize / 18;
  const text = c(entry?.label ?? "", "");
  const a = Math.max(0, Math.min(1, n(entry?.alpha ?? 1, 1)));
  const accent = c(entry?.accent ?? color(pal, "accent", "#FFFFFFFF"), color(pal, "accent", "#FFFFFFFF"));
  return ui.clippedText({
    key: `entry:${entry?.key ?? index}:label`,
    text,
    x: 24,
    y: 6.0,
    w: Math.max(1, pos.w - 29),
    h: Math.max(8, pos.h - 8),
    textClass: cls(font("Inter", fontSize, "bold"), `text-${alpha(color(pal, "text", "#FFFFFFFF"), a)}`),
    color: alpha(color(pal, "text", "#FFFFFFFF"), a),
    measuredWidth: Math.max(1, text.length * 6.4 * widthScale),
    fadeWidth: 7,
    fade: true,
    centerWhenFits: false,
  });
}

function entryNode(p, pal, entry, index) {
  const pos = entryPos(p, index);
  const children = [];
  const text = compactText(p, pal, pos, entry, index);
  if (text) children.push(text);
  return ui.stack({
    key: `entry:${entry?.key ?? index}`,
    style: {
      position: "absolute",
      left: pos.x,
      top: pos.y,
      width: pos.w,
      height: pos.h,
      borderRadius: Math.min(pos.w, pos.h) * 0.24,
    },
  }, children);
}

export function render(ctx) {
  const p = ctx.props || {};
  const pal = p.palette ?? {};
  const w = n(p.width, ctx.width || 24);
  const h = n(p.height, ctx.height || 24);
  const items = Array.isArray(p.items) ? p.items : [];
  return ui.root({
    key: "itemizer",
    style: { width: w, height: h },
  }, items.map((entry, index) => entryNode(p, pal, entry, index)));
}
